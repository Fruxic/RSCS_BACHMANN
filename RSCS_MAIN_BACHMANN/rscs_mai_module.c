...
*           rscs_mai_Init() which spawns the function rscs_mai_bTaskMain
*           as the communication task (b-Task).
*           If rscs_mai_Init returns "successfully", the SMI call
...
*           all application specific work is done in the file rscs_mai_app.c.
*           Only the execution of module specific SMI calls (if used)
*           has to be added to rscs_mai_Main.
*
********************************************************************************
...
SMI_ID *rscs_mai_pSmiId;              /* Id of module-SMI */
SINT32  rscs_mai_Debug = 0;           /* Debug level of module */
SINT32  rscs_mai_AppPrio = 0;         /* Task priority of module */
UINT32  rscs_mai_ModState;            /* Module state */
SEM_ID  rscs_mai_StateSema = 0;       /* Semaphore for halting tasks */
CHAR    rscs_mai_ProfileName[M_PATHLEN_A];    /* Path/Name of config file */
UINT32  rscs_mai_CfgLine = 0;         /* Start line in config file */
CHAR    rscs_mai_AppName[M_MODNAMELEN_A];     /* Instance name of module */
CHAR    rscs_mai_ModuleInfoDesc[SMI_DESCLEN_A];
UINT32  rscs_mai_SviHandle = 0;       /* SVI server handle */
MLOCAL jmp_buf JumpEnv;                 /* Jump Environment for 'longjmp' */
/* The file rscs_mai.ver will be generated by the C++ Developer tool */
CHAR    rscs_mai_Version[M_VERSTRGLEN_A] = {
#include "rscs_mai.ver"
};

/* Functions to be called from outside this file */
SINT32  rscs_mai_Init(MOD_CONF * pConf, MOD_LOAD * pLoad);
...
*        which will be called by SMI after the function rscs_mai_Init()
*        has returned.
*
...
SINT32 rscs_mai_Init(MOD_CONF * pConf, MOD_LOAD * pLoad)
{
    ...
    CHAR    Func[] = "rscs_mai_Init";

    /* Copy profile content to module variables */
    rscs_mai_Debug = pConf->DebugMode;
    rscs_mai_CfgLine = pConf->LineNbr;
    rscs_mai_AppPrio = pConf->TskPrior;
    strncpy(rscs_mai_AppName, pConf->AppName, M_MODNAMELEN);
    rscs_mai_AppName[M_MODNAMELEN] = 0;
    strncpy(rscs_mai_ProfileName, pConf->ProfileName, M_PATHLEN);
    rscs_mai_ProfileName[M_PATHLEN] = 0;
    ...
    do
    {
         ...
         * The variable rscs_mai_SmiId will be initialized by MOD.
         * This ID will be required as parameter to send and receive
         * SMI calls
         */
        if (res_ModParam(rscs_mai_AppName, RSCS_MAI_MINVERS,
                         RSCS_MAI_MAXVERS, RES_UNLIMITUSR, &rscs_mai_pSmiId) != RES_E_OK)
        {
         ...
         */
        if (BaseInit() < 0)
            rscs_mai_ModState = RES_S_ERROR;
        else
            rscs_mai_ModState = RES_S_EOI;

        /*
         * Start the SMI server as task for handling incoming SMI-calls.
         * This task should be in the priority group "Application 2"
         */
        snprintf(TaskName, sizeof(TaskName), "b%s", rscs_mai_AppName);
        TskId = sys_TaskSpawn(rscs_mai_AppName, TaskName, SMI_SRV_PRIO,
        ...
        if (res_ModState(rscs_mai_AppName, rscs_mai_ModState) < 0)
...
MLOCAL SINT32 BaseInit(VOID)
{
    ...
    if (!(rscs_mai_StateSema = semBCreate(SEM_Q_PRIORITY, SEM_EMPTY)))
    {
...
MLOCAL VOID BaseDeinit(VOID)
{
    ...
    /* Inform resource handler about changes of the module state */
    ret = res_ModState(rscs_mai_AppName, rscs_mai_ModState = RES_S_DEINIT);
    if (ret != RES_E_OK)
    ...
    if (rscs_mai_StateSema)
    {
        if (semDelete(rscs_mai_StateSema) < 0)
            LOG_E(0, Func, "Could not delete state semaphore!");
        else
        	rscs_mai_StateSema = 0;
    }
     ...
     */
    rscs_mai_SviSrvDeinit();

    /* De-initialize resources allocated in rscs_mai_AppInit() */
    rscs_mai_AppDeinit();
...
MLOCAL VOID bTaskMain(VOID)
{
    ...
    if (Status == 0)
    {
        /* This branch will be taken after start of RSCS_MAI */
    ...
    else
    {
        ...
        if (BaseInit() < 0)
        {
            ret = res_ModState(rscs_mai_AppName, rscs_mai_ModState = RES_S_ERROR);
            if (ret != RES_E_OK)
        ...
        else
        {
            ...
            ret = res_ModState(rscs_mai_AppName, rscs_mai_ModState = RES_S_RUN);
            if (ret != RES_E_OK)
    ...
    while (1)
    {
        ...
        if (fpSmiReceive)
            Status = fpSmiReceive(rscs_mai_pSmiId, &Msg, WAIT_FOREVER, &UserSessionId);
        else
            Status = 0;
        ...
        if (Msg.Type & SMI_F_CALL)
        {
            ...
            {
                case SMI_PROC_NULL:
                    LOG_I(4, rscs_mai_AppName, "%s: received call SMI_PROC_NULL", Func);
                    RpcNull(&Msg);
                    break;

                case SMI_PROC_DEINIT:
                    LOG_I(4, rscs_mai_AppName, "%s: received call SMI_PROC_DEINIT", Func);
                    RpcDeinit(&Msg);
                    return;             /* quit task completely in this case */

                case SMI_PROC_RESET:
                    LOG_I(4, rscs_mai_AppName, "%s: received call SMI_PROC_RESET", Func);
                    RpcReset(&Msg);
                    break;

                case SMI_PROC_STOP:
                    LOG_I(4, rscs_mai_AppName, "%s: received call SMI_PROC_STOP", Func);
                    RpcStop(&Msg);
                    break;

                case SMI_PROC_RUN:
                    LOG_I(4, rscs_mai_AppName, "%s: received call SMI_PROC_RUN", Func);
                    RpcRun(&Msg);
                    break;

                case SMI_PROC_NEWCFG:
                    LOG_I(4, rscs_mai_AppName, "%s: received call SMI_PROC_NEWCFG", Func);
                    RpcNewCfg(&Msg);
                    break;

                case SMI_PROC_GETINFO:
                    LOG_I(4, rscs_mai_AppName, "%s: received call SMI_PROC_GETINFO", Func);
                    RpcGetInfo(&Msg);
                    break;

                case SMI_PROC_ENDOFINIT:
                    LOG_I(4, rscs_mai_AppName, "%s: received call SMI_PROC_ENDOFINIT", Func);
                    RpcEndOfInit(&Msg);
                    break;

                case SMI_PROC_SETDBG:
                    LOG_I(4, rscs_mai_AppName, "%s: received call SMI_PROC_SETDBG", Func);
                    RpcSetDbg(&Msg);
                    break;
                ...
                case SVI_PROC_SETMULTIBLK:
                    LOG_I(4, rscs_mai_AppName, "%s: received call SVI_PROC_....", Func);
                    /* Pass call to message handler */
                    if (fpSviMsgHandler)
                        fpSviMsgHandler(rscs_mai_SviHandle, &Msg, rscs_mai_pSmiId, UserSessionId);
                    break;

                    /* Not a standard SMI call */
                default:
                    LOG_W(2, rscs_mai_AppName, "%s: received unknown call with SMI id %d", Func, Msg.ProcRetCode);

                    smi_FreeData(&Msg);

                    if (smi_SendReply(rscs_mai_pSmiId, &Msg, SMI_E_PROC, 0, 0) < 0)
...
{
    smi_FreeData(pMsg);
    if (smi_SendReply(rscs_mai_pSmiId, pMsg, SMI_E_OK, 0, 0) < 0)
...
MLOCAL VOID RpcReset(SMI_MSG * pMsg)
{
    ...
    else
    {
        ...
        ret = res_ModState(rscs_mai_AppName, rscs_mai_ModState = RES_S_EOI);
        if (ret != RES_E_OK)
    ...
    /* Send reply */
    smi_FreeData(pMsg);
    if (smi_SendCReply(rscs_mai_pSmiId, pMsg, SMI_E_OK, &Reply, sizeof(Reply)) < 0)
...
*
*        All additional tasks (beside rscs_mai_main()) have to implement the
...
MLOCAL VOID RpcStop(SMI_MSG * pMsg)
{
    ...
    if (rscs_mai_ModState != RES_S_RUN)
    {
        Reply.RetCode = SMI_E_FAILED;
    }
    else
    {
        /* Set module state */
        ret = res_ModState(rscs_mai_AppName, rscs_mai_ModState = RES_S_STOP);
        if (ret != RES_E_OK)
...
MLOCAL VOID RpcRun(SMI_MSG * pMsg)
{
    ...
    if (rscs_mai_ModState != RES_S_STOP)
    {
...
MLOCAL VOID RpcNewCfg(SMI_MSG * pMsg)
{
    ...
    if (rscs_mai_ModState == RES_S_STOP || rscs_mai_ModState == RES_S_RUN ||
        rscs_mai_ModState == RES_S_ERROR)
    {
        ...
        if (rscs_mai_CfgRead() || rscs_mai_AppEOI())
        {
            ret = res_ModState(rscs_mai_AppName, rscs_mai_ModState = RES_S_ERROR);
            if (ret != RES_E_OK)
...
MLOCAL VOID RpcDeinit(SMI_MSG * pMsg)
{
     ...
     * because rscs_mai_SmiId will become invalid
    ...
    if (res_ModDelete(rscs_mai_AppName) != RES_E_OK)
...
MLOCAL VOID RpcEndOfInit(SMI_MSG * pMsg)
{
    ...
    if (rscs_mai_ModState != RES_S_EOI)
    {
...
MLOCAL VOID RpcSetDbg(SMI_MSG * pMsg)
{
    ...
    rscs_mai_Debug = pCall->DebugMode;
    Reply.RetCode = SMI_E_OK;
...
MLOCAL VOID RpcGetInfo(SMI_MSG * pMsg)
{
    ...
    if (!pReply)
    {
        ...
        if (smi_SendReply(rscs_mai_pSmiId, pMsg, SMI_E_ARGS, 0, 0) < 0)
    ...
    strncpy(pReply->Name, rscs_mai_AppName, M_MODNAMELEN);
    pReply->Name[M_MODNAMELEN] = 0;

    strncpy(pReply->Desc, rscs_mai_ModuleInfoDesc, sizeof(pReply->Desc));
     ...
     */
    sys_GetVersion(&Version, rscs_mai_Version);
    pReply->VersType = Version.Type;
    memcpy(&pReply->VersCode, Version.Code, sizeof(pReply->VersCode));

    pReply->State = rscs_mai_ModState;
    pReply->DebugMode = rscs_mai_Debug;
    pReply->RetCode = SMI_E_OK;

    /* Send reply */
    if (smi_SendReply(rscs_mai_pSmiId, pMsg, SMI_E_OK, pReply, sizeof(*pReply)) < 0)
