...
/* Project includes */
#include "rscs_mai.h"
#include "rscs_mai_e.h"
#include "rscs_mai_int.h"

/* Functions: administration, to be called from outside this file */
SINT32  rscs_mai_AppEOI(VOID);
VOID    rscs_mai_AppDeinit(VOID);
SINT32  rscs_mai_CfgRead(VOID);
SINT32  rscs_mai_SviSrvInit(VOID);
VOID    rscs_mai_SviSrvDeinit(VOID);

/* Functions: administration, to be called only from within this file */
MLOCAL VOID rscs_mai_CfgInit(VOID);
...
RSCS_MAI_BASE_PARMS rscs_mai_BaseParams;
...
MLOCAL TASK_PROPERTIES TaskProperties_aControl = {
    "aRSCS_MAI_Ctrl",                 /* unique task name, maximum length 14 */
...
MLOCAL SVI_GLOBVAR SviGlobVarList[] = {
    {"CycleCounter", SVI_F_INOUT | SVI_F_UINT32, sizeof(UINT32), (UINT32 *) & CycleCount, 0, NULL,
     NULL}
    ,
    {"ModuleVersion", SVI_F_OUT | SVI_F_STRING, sizeof(rscs_mai_Version),
     (UINT32 *) rscs_mai_Version, 0, NULL, NULL}
};
...
SINT32 rscs_mai_AppEOI(VOID)
{
    ...
    do
    {
        ...
        snprintf(rscs_mai_ModuleInfoDesc, sizeof(rscs_mai_ModuleInfoDesc), "TODO: set application specific module info string");
...
*        For general configuration data, rscs_mai_CfgParams is being used.
*        Being called by rscs_mai_CfgRead.
...
MLOCAL SINT32 Task_CfgRead(VOID)
{
    ...
    snprintf(section, sizeof(section), rscs_mai_BaseParams.AppName);
    ...
    for (idx = 0; idx < NbOfTasks; idx++)
    {
        ...
        ret = pf_GetStrg(section, group, key, "", (CHAR *) & TmpStrg, sizeof(TmpStrg),
                         rscs_mai_BaseParams.CfgLine, rscs_mai_BaseParams.CfgFileName);
        ...
        else
        {
            ...
            {
                TaskList[idx]->Priority = rscs_mai_BaseParams.DefaultPriority;
...
MLOCAL SINT32 Task_CreateAll(VOID)
{
    ...
    for (idx = 0; idx < NbOfTasks; idx++)
    {
        ...
        if (TaskList[idx]->WDogRatio > 0)
        {
            ...
            TaskList[idx]->WdogId = sys_WdogCreate(rscs_mai_AppName, wdogtime_us);
        ...
        if (strlen(TaskList[idx]->Name) < 1)
            snprintf(TaskList[idx]->Name, sizeof(TaskList[idx]->Name), "a%s_%d", rscs_mai_AppName, idx + 1);
        ...
        TaskList[idx]->TaskId = sys_TaskSpawn(rscs_mai_AppName, TaskName,
...
MLOCAL VOID Task_DeleteAll(VOID)
{
    ...
    do
    {
        ...
        if (AllTasksQuitted)
        {
            if (rscs_mai_BaseParams.DebugMode & APP_DBG_INFO1)
...
MLOCAL SINT32 Task_InitTiming_Sync(TASK_PROPERTIES * pTaskData)
{
    ...
    pTaskData->SyncSessionId = mio_StartSyncSession(rscs_mai_AppName);
...
MLOCAL VOID Task_WaitCycle(TASK_PROPERTIES * pTaskData)
{
     ...
     */
    if ((rscs_mai_ModState == RES_S_STOP) || (rscs_mai_ModState == RES_S_EOI))
    {
         ...
         */
        semTake(rscs_mai_StateSema, WAIT_FOREVER);
    }
}
...
MLOCAL VOID rscs_mai_CfgInit(VOID)
{
    ...
    strncpy(rscs_mai_BaseParams.CfgFileName, rscs_mai_ProfileName, M_PATHLEN);

    /* Application name */
    strncpy(rscs_mai_BaseParams.AppName, rscs_mai_AppName, M_MODNAMELEN);

    /* Line number in configuration file (used as start line for searching) */
    rscs_mai_BaseParams.CfgLine = rscs_mai_CfgLine;

    /* Worker task priority from module base parameters (BaseParms) */
    rscs_mai_BaseParams.DefaultPriority = rscs_mai_AppPrio;

    /* Debug mode from module base parameters (BaseParms) */
    rscs_mai_BaseParams.DebugMode = rscs_mai_Debug;
...
SINT32 rscs_mai_SviSrvInit(VOID)
{
    ...
    UINT32  i;
    CHAR    Func[] = "rscs_mai_SviSrvInit";

    /* If there are any SVI variables to be exported */
    if (NbOfGlobVars)
    {
        /* Initialize SVI-handler */
        rscs_mai_SviHandle = svi_Init(rscs_mai_AppName, 0, 0);
        if (!rscs_mai_SviHandle)
        {
    ...
    {
        ret = svi_AddGlobVar(rscs_mai_SviHandle, SviGlobVarList[i].VarName,
...
/**
********************************************************************************
* @brief Frees SVI server resources according to rscs_mai_SviSrvInit()
...
VOID rscs_mai_SviSrvDeinit(VOID)
{
    ...
    if (svi_DeInit(rscs_mai_SviHandle) < 0)
        LOG_E(0, "rscs_mai_SviSrvDeinit", "Could not de-initialize SVI server");
